...
On macOS, `--local` is auto-detected. The script installs Data Machine, OpenCode, agent skills, and optionally Kimaki — no infrastructure, no root, no systemd.

Start your agent:

```bash
cd ~/Studio/my-wordpress-website && kimaki    # Discord
cd ~/Studio/my-wordpress-website && opencode  # Terminal only
```
...
```bash
# Local: WordPress Studio + DM + Discord
...
# VPS: Full setup (WordPress + DM + Discord)
...
wp-opencode defaults to running the agent as `root` on VPS installs. On a single-purpose agent VPS, root keeps things simple:
...
The default chat bridge. On VPS, wp-opencode installs post-upgrade hooks that:
...
Use `--chat telegram` to install [opencode-telegram](https://github.com/grinev/opencode-telegram-bot) instead. This sets up two systemd services: `opencode-serve` (the OpenCode HTTP server) and `opencode-telegram` (the bot that connects to it). VPS only.
