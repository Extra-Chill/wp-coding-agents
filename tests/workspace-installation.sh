...
case "$*" in
  ...
  "config show /settings/worktree_provider_lifecycle/dmc") test -f "$HOMEBOY_LIFECYCLE" ;;
  "config remove /settings/worktree_provider_lifecycle/dmc") rm -f "$HOMEBOY_LIFECYCLE" ;;
  *) exit 1 ;;
esac
SH
chmod +x "$BIN/homeboy"
...
HOMEBOY_LIFECYCLE="$TMP/lifecycle"
touch "$HOMEBOY_PROVIDER" "$HOMEBOY_LIFECYCLE"
mkdir -p "$SITE/wp-content/mu-plugins"
printf '%s\n' "Route DMC's canonical worktree abilities through Homeboy's native lifecycle." 'Generated by wp-coding-agents. Local edits will be overwritten.' > "$SITE/wp-content/mu-plugins/wp-coding-agents-homeboy-worktrees.php"
export HOMEBOY_LOG HOMEBOY_PROVIDER HOMEBOY_LIFECYCLE PATH
UPDATED_ITEMS=()
configure_homeboy_worktree_ownership
test ! -e "$HOMEBOY_PROVIDER" || fail "Homeboy retained a DMC provider"
test ! -e "$HOMEBOY_LIFECYCLE" || fail "Homeboy retained a DMC lifecycle setting"
...
printf 'operator-owned adapter\n' > "$SITE/wp-content/mu-plugins/wp-coding-agents-homeboy-worktrees.php"
if configure_homeboy_worktree_ownership; then fail "unknown adapter cleanup reported success"; fi
test -e "$SITE/wp-content/mu-plugins/wp-coding-agents-homeboy-worktrees.php" || fail "unknown adapter was removed"
