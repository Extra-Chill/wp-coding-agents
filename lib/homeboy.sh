...
sync_homeboy_project_components() {
  ...
  while IFS= read -r repository; do
    ...
    if [ ! -d "$repository" ]; then
      warn "  skipped $name: declared repository does not exist"
      continue
    fi
    if [ "$repository" = "${SITE_PATH:-}" ]; then
      warn "  skipped $name: site project root"
      continue
    fi
    if [[ "$name" == *"@"* ]]; then
      warn "  skipped $name: worktree skipped"
      continue
    fi
    if [ "${DRY_RUN:-false}" = true ]; then
      echo -e "${BLUE}[dry-run]${NC} homeboy project components attach-path $project_id $repository"
      continue
    fi
    if homeboy_run project components attach-path "$project_id" "$repository" >/dev/null; then
      log "  attached $name"
    else
      warn "  failed $name: homeboy attach-path failed"
    fi
  done < <(source_policy_workspace_repositories)
}

homeboy_retired_worktree_adapter_file() {
  printf '%s' "$SITE_PATH/wp-content/mu-plugins/wp-coding-agents-homeboy-worktrees.php"
}

homeboy_retired_worktree_adapter_remove() {
  local file
  file="$(homeboy_retired_worktree_adapter_file)"
  [ -e "$file" ] || return 0

  if ! grep -q "Route DMC's canonical worktree abilities through Homeboy's native lifecycle" "$file" || \
     ! grep -q 'Generated by wp-coding-agents. Local edits will be overwritten.' "$file"; then
    if [ "${DRY_RUN:-false}" = true ]; then warn "Would preserve unknown Homeboy worktree adapter at $file (installer provenance is not confirmed)"; else warn "Preserving unknown Homeboy worktree adapter at $file (installer provenance is not confirmed)"; fi
    return 1
  fi

  if [ "${DRY_RUN:-false}" = true ]; then
    echo -e "${BLUE}[dry-run]${NC} Would remove installer-owned retired Homeboy worktree adapter at $file"
    return 0
  fi
...
configure_homeboy_worktree_ownership() {
  local status=0
  # This retired adapter projected DMC ability callbacks into Homeboy. Homeboy
  ...
  if homeboy_run config show /worktree_providers/dmc >/dev/null 2>&1; then homeboy_handle_failure "Circular /worktree_providers/dmc configuration remains authoritative."; status=1; fi
  if homeboy_run config show /settings/worktree_provider_lifecycle/dmc >/dev/null 2>&1; then homeboy_handle_failure "Retired /settings/worktree_provider_lifecycle/dmc configuration remains authoritative."; status=1; fi
  if [ "${HOMEBOY_MODE:-auto}" = "disabled" ]; then
    log "Removed retired Homeboy worktree state (--no-homeboy)"
  fi
  return "$status"
}
